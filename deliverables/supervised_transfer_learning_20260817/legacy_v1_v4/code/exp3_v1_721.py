...
def try_improved_models(source_power='1.0kW', target_power='3.0kW', base_path=str(DATASETS_DIR), seed=42):
    ...
    print('\nLoading data with time-blocked 70:20:10 split (train:test:val)...')
    ...
    for i, cfg_dict in enumerate(configs):
        ...
        # Track best model
        # Configuration selection must use validation data, never the test set.
        if target_results_after['val'] > best_target_acc:
            best_target_acc = target_results_after['val']
            best_model = model
